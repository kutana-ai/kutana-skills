...
| `share-local-dev-server` | "share screen", "demo", "dev server", "show this in the meeting" | (Coming soon) Provisions a Cloudflare Tunnel for the agent's local dev server and embeds a live Browserbase view in the meeting's ArtifactPanel — the unified-tunnel architecture. |
